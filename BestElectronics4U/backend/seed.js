  ...
  for (const name of segments.map(s => s.trim())) {
    ...
    if (rows.length > 0) {
      ...
      const [result] = await db.execute(
        'INSERT IGNORE INTO category (category_name, parent_id) VALUES (?, ?)',
        [name, parentId]
      );
...
async function runSeed() {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      console.log("🌱 Seeder: Connected to MySQL");
      ...
      fs.createReadStream('./products.csv')
        ...
        .on('error', (streamError) => {
          console.error('❌ Seeder: CSV stream error:', streamError);
          if (connection) {
            connection.end().catch(e => console.error("❌ Seeder: Failed to close connection on stream error", e));
          }
          reject(streamError);
        })
        .on('end', async () => {
          console.log(`🌱 Seeder: CSV loaded: ${products.length} rows`);
          let insertedCount = 0;
          ...
          for (const product of products) {
            ...
            if (isNaN(dp) || isNaN(ap)) {
              console.warn(`🌱 Seeder: Skipping ${product_name} due to invalid prices`);
              continue;
            }
            ...
            const [shopRows] = await connection.execute('SELECT shop_id FROM shop WHERE shop_id = ?', [user_id]);
            if (shopRows.length === 0) {
              await connection.execute(
                'INSERT IGNORE INTO shop (shop_id, shop_name, shop_address) VALUES (?, ?, ?)',
            ...
            if (review_id && review_title && review_content) {
          ...
          console.log(`🌱 Seeder: ${insertedCount} products processed for seeding.`);
          await connection.end();
          console.log("🌱 Seeder: Connection ended. Seed process complete.");
          resolve();
        });
    } catch (error) {
      console.error('❌ Seeder: General error in runSeed:', error);
      if (connection) {
        connection.end().catch(e => console.error("❌ Seeder: Failed to close connection on general error", e));
      }
      reject(error);
    }
  });
